...
void filter2WeightFilter(boost::shared_ptr<DictEqualityFilter> eqFilter,DictWeightEqualityFilter* eqWeightFilter)
{
  if (eqFilter)
  {
    for (auto &x : *eqFilter)
    {
      ...
      string eqWeightArray(x.length()<16?16:x.length());
...
#endif
{
  ...
  filter2WeightFilter(eqFilter,&eqWeightFilter)
